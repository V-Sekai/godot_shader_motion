...
func _prepare_animation_tracks(base_animation: Animation, bone_names):
	for bone in range(0, int(ShaderMotionHelpers.MecanimBodyBone.LastBone)):
...
func _ready():
	var should_stop: bool = NodeHelpers.stop_if_any_is_null(self, [analyzed_pixels], "Tile Frames to Animation")
	if should_stop:
		return
	...
	ResourceSaver.save(animation, "res://shader_motion/animations/exported_animation_faster_onready.tres")
	get_tree().quit()


func add_frames_to_animation(animation: Animation, frames_to_convert: TileFrames):
...
func _add_animation_frame(
	...
	for bone in range(0, int(ShaderMotionHelpers.MecanimBodyBone.LastBone)):
		...
		animation.rotation_track_insert_key(animation_track_index, animation_time, godot_rotation)
