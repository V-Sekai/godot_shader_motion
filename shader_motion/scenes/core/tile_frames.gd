...
func get_motion_data_for(frame_time: float) -> ShaderMotionHelpers.ParsedMotions:
	...
	if frame_tiles.is_empty():
		printerr("[TileFrames.get_motion_data_for] Failed to get the frames at time %f" % frame_tiles)
		return null

	return ShaderMotionHelpers.ParsedMotions.generate_from(frame_tiles, get_tiles_colors_from_slot)
